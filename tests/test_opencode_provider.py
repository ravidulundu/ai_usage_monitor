...
from core.ai_usage_monitor.providers.opencode import _local_auth_type, collect_opencode, normalize_workspace_id, parse_subscription_text
...
class OpenCodeProviderTests(unittest.TestCase):
    ...
    def test_collect_opencode_uses_local_cli_mode_when_install_detected(self):
        with mock.patch("core.ai_usage_monitor.providers.opencode._has_local_opencode_install", return_value=True):
            with mock.patch("core.ai_usage_monitor.providers.opencode.scan_opencode_local_usage") as usage_mock:
                usage_mock.return_value = mock.Mock(session_tokens=1234, last_30_days_tokens=5678)
                with mock.patch("core.ai_usage_monitor.providers.opencode._local_auth_type", return_value="api"):
    ...
    def test_collect_opencode_auto_prefers_web_quota_when_cookie_available(self):
        with mock.patch("core.ai_usage_monitor.providers.opencode._cookie_header", return_value=("auth=1", "manual")):
            with mock.patch("core.ai_usage_monitor.providers.opencode._fetch_server_text") as fetch_mock:
    ...
    def test_local_auth_type_reads_opencode_auth_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            auth_file.write_text(json.dumps({"opencode": {"type": "api", "key": "test"}}))
