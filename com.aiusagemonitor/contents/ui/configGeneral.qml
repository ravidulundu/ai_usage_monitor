...
QQC2.ScrollView {
    ...
    property var stagedConfig: ({ "version": 1, "refreshInterval": 60, "providers": [] })
    ...
    function cloneConfig(config) {
        return JSON.parse(JSON.stringify(config || { "version": 1, "refreshInterval": 60, "providers": [] }))
    }

    function syncStagedPayload() {
        cfg_sharedConfigPayload = JSON.stringify(stagedConfig)
    }

    function setProviderField(providerId, field, value) {
        var config = cloneConfig(stagedConfig)
        if (!config.providers)
            config.providers = []
        var found = false
        ...
        stagedConfig = config
        sharedProviders = config.providers
        syncStagedPayload()
    }
    ...
    function saveConfig() {
        configRunner.connectSource(
            ...
            + " config-save-json "
            + shellQuote(cfg_sharedConfigPayload)
        )
    }
        ...
        onNewData: function(sourceName, data) {
            ...
            try {
                ...
                if (payload.config && payload.config.providers) {
                    ...
                    configRoot.stagedConfig = cloneConfig(payload.config)
