---
    ...
    - ``core/ai_usage_monitor/**/*.py`, `tests/test_*provider*.py`` altında dosya okunuyor, düzenleniyor veya gözden geçiriliyorsa
    - Kullanıcı error handling, ilgili bounded context veya komşu alanları anıyorsa
    ...
    ### Rule 1: Boundary’de yakala ve normalize et

**Rule**: Provider collector, CLI ve presentation boundary’lerinde hata shape’i normalize edilir.

**Rationale**: Aynı hata için farklı UI davranışını engeller.
...
```
### Rule 2: Operational ve programmer error ayrımı yap

**Rule**: Beklenen auth/network eksikliği ile bug aynı sepete atılmaz.

**Rationale**: Yanlış alert ve yanlış kullanıcı mesajını önler.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 3: Bir kez logla

**Rule**: Aynı hata zincirini her layer’da tekrar loglama.

**Rationale**: Noise ve secret sızıntısı riskini azaltır.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 4: Fallback metni source-aware olmalı

**Rule**: `auth invalid`, `local unavailable`, `source switched` gibi nedenler generic hataya indirgenmez.

**Rationale**: Kullanıcı tanısı kolaylaşır.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 5: Recovery yoksa explicit unavailable dön

**Rule**: Sahte sıfır değer veya stale metric gösterme.

**Rationale**: Yanlış usage güvenini engeller.
...
```
    ...
    **The danger**: Gerçek hata durumunda sahte sıfır usage dönmek.
...
# -> hides failure as fake data
    ...
    - [ ] Hata metinleri local path, cookie adı veya raw HTTP gövdesi göstermemelidir.
    ...
    |----------|-----------|----------|-------------|----------|
    | Remote API timeout | urllib timeout veya retry branch | Unavailable state + kısa neden + cached view mümkünse koru | Kullanıcı geçici veri eksikliği görür | High |
