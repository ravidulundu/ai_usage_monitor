---
    ...
    - ``tools/`, health checks, provider debug helpers, logs` altında dosya okunuyor, düzenleniyor veya gözden geçiriliyorsa
    - Kullanıcı observability, ilgili bounded context veya komşu alanları anıyorsa
    ...
    ### Rule 1: Boundary logları yapılandırılmış ve kısa tut

**Rule**: Provider/source/identity olayları grep edilebilir olmalı.

**Rationale**: Yerel tanı süresini düşürür.
...
```
### Rule 2: Health checks product contract’a bağlı olsun

**Rule**: Sadece syntax değil lifecycle ve contract drift de ölçülür.

**Rationale**: Sessiz UI kırıklarını önler.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 3: Slow path ölçümü komutla doğrulanabilir olsun

**Rule**: Performance bütçeleri ölçüm komutuna bağlı yazılır.

**Rationale**: “Hızlı olmalı” belirsizliğini kaldırır.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 4: Debug çıktısı secret-redacted olmalı

**Rule**: Tanı için yeterli ama güvenli minimum context taşı.

**Rationale**: Yerel destek loglarında sızıntıyı önler.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 5: User-facing issue önce test/log ile yeniden üret

**Rule**: Varsayım yerine kanıtla ilerle.

**Rationale**: Yanlış kök neden analizini azaltır.
...
```
    ...
    **Implementation**:
    ```python
    event = {'action': 'provider_refresh', 'provider': 'codex', 'duration_ms': 42}
print(event)
    ```
    ...
    **The danger**: Kök neden çıkarmaya yardım etmeyen serbest debug metni.
    **The fix**: Canonical boundary’ye geri taşı ve regression test ekle.

    ```python
    print('something broke somewhere')
# no provider, no action, no duration
    ```
    ...
    - [ ] Observability alanında token, cookie ve auth file path redaction kuralı asla delinmez.
    ...
    |----------|-----------|----------|-------------|----------|
    | Health gate geçmedi ama sebep belirsiz | CI/local output noisy | Check adını ve fail mesajını domain-specific yap | Fix süresi uzar | Medium |
    ...
    **Code**:
    ...
    |-------------|-------|--------|
    | Depends on | `error-handling, performance-optimization, provider-patterns` | Bu skill aynı bounded context kararlarını devralır |
