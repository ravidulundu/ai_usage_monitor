---
    ...
    - ``config.py`, `sources/`, `presentation/`, `providers/`` altında dosya okunuyor, düzenleniyor veya gözden geçiriliyorsa
    - Kullanıcı data validation, ilgili bounded context veya komşu alanları anıyorsa
    ...
    ### Rule 1: Boundary’de validate et

**Rule**: Config, external API, file parser ve CLI input sınırlarında shape doğrulaması yap.

**Rationale**: Trusted iç kodun sade kalmasını sağlar.
...
```
### Rule 2: Trusted iç kodda tekrar validate etme

**Rule**: Bir kez normalize edilen veri typed contract ile taşınır.

**Rationale**: Çift validasyon karmaşasını önler.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 3: Enum-benzeri stringleri canonical helper ile çöz

**Rule**: `resolvedSource`, `preferredSource` ve reason code’lar serbest string olmaz.

**Rationale**: Semantik drift’i önler.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 4: Optional alanları explicit taşı

**Rule**: Eksik alan `None` ile görünür olur; yok sayılmaz.

**Rationale**: Renderer fallback buglarını azaltır.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 5: Validation hatasını domain hatasına çevir

**Rule**: Parse exception çıplak kalmaz.

**Rationale**: UI-safe hata üretimini korur.
...
```
    ...
    **The danger**: UI ve core arasında serbest string reason taşımak.
...
# -> invalid source leaks deep into VM
    ...
    - [ ] Validation başarısızlığında raw payload’ı tamamen loglamak yerine redacted özet kullan.
    ...
    |----------|-----------|----------|-------------|----------|
    | Bozuk config.json | normalize/load testleri veya JSON decode failure | Default+safe merge ya da kullanıcıya net config error | Ayarlar yüklenmez | High |
