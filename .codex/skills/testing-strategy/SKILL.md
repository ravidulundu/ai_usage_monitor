---
    ...
    - ``tests/`, `Makefile`, `.github/workflows/`, `tasks/todo.md`` altında dosya okunuyor, düzenleniyor veya gözden geçiriliyorsa
    - Kullanıcı testing strategy, ilgili bounded context veya komşu alanları anıyorsa
    ...
    ### Rule 1: Bug fix önce regression test

**Rule**: Hata yeniden üretilemeden fix yazma.

**Rationale**: Sessiz regresyon riskini düşürür.
...
```
### Rule 2: Contract surface testlerini ayrı tut

**Rule**: `popup-vm`, `config-ui`, `sourceModel`, provider state shape testleri bağımsız görünür olmalı.

**Rationale**: UI drift tespitini hızlandırır.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 3: Deterministik test yaz

**Rule**: Saat, random ve home dir bağımlılıklarını sabitle.

**Rationale**: Flaky CI’ı önler.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 4: Health gate final kapıdır

**Rule**: Hedefli pytest yetmez; son turda `make health-ci PYTHON=python` çalışmalıdır.

**Rationale**: Cross-layer kırıkları yakalar.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 5: Provider testleri mutlu yol + auth yok + hata yolu içermeli

**Rule**: Tek mutlu yol testi yeterli değildir.

**Rationale**: Collector dayanıklılığını ölçer.
...
```
    ...
    **Implementation**:
    ```python
    def test_popup_provider_keeps_metrics_visible_for_stable_fallback_source() -> None:
    assert True
    ...
    **The danger**: Davranış doğrulamayan testler sahte güven üretir.
    **The fix**: Canonical boundary’ye geri taşı ve regression test ekle.

    ```python
    def test_provider_smoke():
    collect_provider()
# no assertions
    ```
    ...
    - [ ] Auth ve identity alanlarına dokunan her değişiklikte izinsiz/stale-data senaryosu test edilmeli.
    ...
    |----------|-----------|----------|-------------|----------|
    | CI sadece subset testle yeşil | health-ci kırılır | Son kapıda tam gate çalıştır | PR merge gecikir | Medium |
    ...
    **Code**:
    ...
    |-------------|-------|--------|
    | Depends on | `project-architecture, python-standards, error-handling, data-validation` | Bu skill aynı bounded context kararlarını devralır |
