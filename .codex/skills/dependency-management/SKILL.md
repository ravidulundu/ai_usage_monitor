---
    ...
    - ``requirements-dev.txt`, `package.json`, `.pre-commit-config.yaml`, archived providers` altında dosya okunuyor, düzenleniyor veya gözden geçiriliyorsa
    - Kullanıcı dependency management, ilgili bounded context veya komşu alanları anıyorsa
    ...
    ### Rule 1: Yeni dependency için bakım ve yüzey gerekçesi yaz

**Rule**: Her paket tasks/review veya plan notunda gerekçelendirilir.

**Rationale**: Supply-chain ve bakım yükünü kontrol altında tutar.
...
```
### Rule 2: Runtime ile dev dependency’yi karıştırma

**Rule**: Tooling paketi uygulama bağımlılığı gibi davranmaz.

**Rationale**: Paket yüzeyini küçük tutar.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 3: Archived provider için yeni dependency ekleme

**Rule**: Dormant kod aktif roadmap gerekçesi olmadan genişletilmez.

**Rationale**: Ölü yüzeyin maliyetini artırmaz.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 4: Version bump’i gate ile doğrula

**Rule**: Lint/test/type/health zinciri kırılmadan update yapılmaz.

**Rationale**: Sessiz toolchain drift’i önler.
...
# -> Consequence: typed contract ve bakım izi zayıflar.
```
### Rule 5: Bir işi standart kütüphane çözüyorsa yeni paket alma

**Rule**: Özellikle Python core için önce stdlib bak.

**Rationale**: Gereksiz attack surface’i azaltır.
    ...
    # package.json yalnız JS tooling içerir
{
  "devDependencies": {
    ...
    **The danger**: Küçük bir string/path işini yeni paketle çözmek.
...
# no plan, no tests, no security review
    ...
    - [ ] Dependency değişikliğinde audit ve changelog okunmadan merge yapılmaz.
    ...
    |----------|-----------|----------|-------------|----------|
    | Toolchain upgrade health-ci kırdı | lint/type/test fail | Sürümü geri al veya uyumluluk patch’i tamamla | Geliştirici akışı durur | Medium |
