...
def collect_warp(settings: dict | None = None) -> tuple[dict, ProviderState]:
    ...
    try:
        ...
        used_pct = 0.0 if is_unlimited or request_limit <= 0 else min(100.0, max(0.0, (used / request_limit) * 100.0))
        ...
        state = ProviderState(
            ...
            authenticated=True,
            source="api",
            primary_metric=MetricWindow("Credits", 0 if is_unlimited else used_pct, reset_time),
            extras={"plan": "Unlimited" if is_unlimited else f"{int(used)}/{int(request_limit)}"},
        )
        return legacy, state
    except urllib.error.HTTPError as err:
        legacy = {"installed": True, **classify_http_failure("warp", err.code, read_http_error_body(err))}
