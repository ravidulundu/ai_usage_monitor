...
DESCRIPTOR = ProviderDescriptor(
    ...
    branding=ProviderBranding(
        icon_key="ollama", asset_name="ollama.svg", color="#888888", badge_text="OL"
    ),
    status_page_url="https://status.ollama.com/",
    usage_dashboard_default_url="https://ollama.com/",
    usage_dashboard_by_source=(("web", "https://ollama.com/"),),
...
def parse_ollama_html(html: str) -> dict[str, Any]:
    lower = html.lower()
    ...
    plan = _first_capture(
        html, r"Cloud Usage\s*</span>\s*<span[^>]*>([^<]+)</span>", re.DOTALL
    )
    email = _first_capture(html, r'id="header-email"[^>]*>([^<]+)<', re.DOTALL)

    def usage_block(labels: str | list[str]) -> dict[str, Any] | None:
        ...
        for label in labels:
            ...
            used = _first_capture(
                window, r"([0-9]+(?:\.[0-9]+)?)\s*%\s*used", re.IGNORECASE
            )
            if used is None:
                used = _first_capture(
                    window, r"width:\s*([0-9]+(?:\.[0-9]+)?)%", re.IGNORECASE
                )
            if used is None:
                continue
...
def collect_ollama(
    ...
    cookie_header = cookie_header_from_settings(
        settings, env_keys=("OLLAMA_COOKIE_HEADER",)
    )
    source_label = "manual"
    ...
    try:
        ...
        state = ProviderState(
            ...
            primary_metric=MetricWindow(
                "Session", session["used"], session.get("reset")
            )
            if session
            else None,
            secondary_metric=MetricWindow("Weekly", weekly["used"], weekly.get("reset"))
            if weekly
            else None,
            extras={
                "plan": parsed.get("plan") or "",
                "model": parsed.get("email") or source_label,
            },
        )
        return legacy, state
        ...
        legacy = {
            "installed": True,
            **classify_http_failure("ollama", err.code, read_http_error_body(err)),
        }
