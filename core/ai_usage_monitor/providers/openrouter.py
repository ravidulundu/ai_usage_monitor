...
DESCRIPTOR = ProviderDescriptor(
    ...
    source_modes=("api",),
    config_fields=(
        ProviderConfigField("apiKey", "API Key", secret=True, placeholder="sk-or-v1-..."),
        ProviderConfigField("apiUrl", "API URL", placeholder="https://openrouter.ai/api/v1"),
    ),
    branding=ProviderBranding(icon_key="openrouter", asset_name="openrouter.svg", color="#6467F2", badge_text="OR"),
)


def _get_setting(settings: dict | None, key: str) -> str | None:
    if settings and isinstance(settings.get(key), str) and settings.get(key).strip():
...
def _base_url(settings: dict | None) -> str:
    return (_get_setting(settings, "apiUrl") or os.environ.get("OPENROUTER_API_URL") or "https://openrouter.ai/api/v1").rstrip("/")


def collect_openrouter(settings: dict | None = None) -> tuple[dict, ProviderState]:
    ...
    headers = {
        ...
        "HTTP-Referer": os.environ.get("OPENROUTER_HTTP_REFERER", "https://github.com/osmandulundu/ai-usage-monitor"),
    ...
    try:
        ...
        used_pct = 0.0 if total_credits <= 0 else min(100.0, max(0.0, (total_usage / total_credits) * 100.0))
        ...
        legacy = {"installed": True, **classify_http_failure("openrouter", err.code, read_http_error_body(err))}
