...
def _parse_mmdd_reset(value: str) -> str | None:
    try:
        ...
        if candidate < now:
            candidate = datetime(now.year + 1, int(month), int(day), tzinfo=timezone.utc)
...
def parse_kiro_output(output: str) -> dict:
    ...
    resets_at = None
    percent_match = re.search(r"(\d+)%\s*\(resets on (\d{2}/\d{2})\)", stripped, re.IGNORECASE)
    if percent_match:
        credits_percent = float(percent_match.group(1))
        resets_at = _parse_mmdd_reset(percent_match.group(2))

    credits_ratio_match = re.search(r"\((\d+(?:\.\d+)?) of (\d+(?:\.\d+)?) covered in plan\)", stripped, re.IGNORECASE)
    ...
    if credits_percent is None:
        raise ValueError("No recognizable usage patterns found. Kiro CLI output format may have changed.")
...
def collect_kiro(settings: dict | None = None) -> tuple[dict, ProviderState]:
    if shutil.which("kiro-cli") is None:
        return {"installed": False}, ProviderState(id=DESCRIPTOR.id, display_name=DESCRIPTOR.display_name, installed=False, source="cli")
    ...
    try:
        ...
        if whoami.returncode != 0 or "not logged in" in combined or "login required" in combined:
            raise PermissionError("Not logged in to Kiro. Run 'kiro-cli login' first.")
        ...
        bonus_metric = None
        if parsed["bonus_used"] is not None and parsed["bonus_total"] and parsed["bonus_total"] > 0:
            bonus_pct = (parsed["bonus_used"] / parsed["bonus_total"]) * 100.0
            bonus_reset = None
            if parsed["bonus_days"] is not None:
                bonus_reset = (datetime.now(timezone.utc) + timedelta(days=parsed["bonus_days"])).isoformat()
        ...
        state = ProviderState(
            ...
            authenticated=True,
            source="cli",
            primary_metric=MetricWindow("Monthly", parsed["credits_percent"], parsed["resets_at"]),
            secondary_metric=bonus_metric,
            extras={"plan": parsed["plan_name"]},
        )
        return legacy, state
    except PermissionError as err:
        legacy = {"installed": True, "authenticated": False, "error": str(err), "fail_reason": "auth_required"}
        state = ProviderState(
            id=DESCRIPTOR.id,
            display_name=DESCRIPTOR.display_name,
            installed=True,
        ...
        legacy = {"installed": True, "error": "Kiro CLI timed out.", "fail_reason": "timeout"}
        state = ProviderState(
            id=DESCRIPTOR.id,
            display_name=DESCRIPTOR.display_name,
            installed=True,
        ...
        legacy = {"installed": True, "error": f"Failed to parse Kiro usage: {err}", "fail_reason": "parse_error"}
        state = ProviderState(
            id=DESCRIPTOR.id,
            display_name=DESCRIPTOR.display_name,
            installed=True,
