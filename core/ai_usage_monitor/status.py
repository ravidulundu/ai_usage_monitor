...
def _read_json(url: str, timeout: int = 5):
...
def fetch_google_workspace_status(product_id: str, public_url: str | None = None) -> dict | None:
    try:
        incidents = _read_json("https://www.google.com/appsstatus/dashboard/incidents.json")
    except Exception:
        return None

    active = []
    for incident in incidents:
        current_products = incident.get("currently_affected_products") or incident.get("currentlyAffectedProducts") or []
        all_products = incident.get("affected_products") or incident.get("affectedProducts") or []
    ...
    def sort_key(incident: dict):
        update = incident.get("most_recent_update") or incident.get("mostRecentUpdate") or {}
        updated_at = update.get("when") or incident.get("modified") or incident.get("begin") or ""
        try:
    ...
    return {
        "indicator": _workspace_indicator(update.get("status") or best.get("status_impact") or best.get("statusImpact"), best.get("severity")),
        "description": _workspace_summary(update.get("text") or best.get("external_desc") or best.get("externalDesc")),
